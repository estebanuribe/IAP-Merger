...
def get_sleep_time():
	...
	if new_sleep_time == last_sleep_time or new_sleep_time == 0 or new_sleep_time > SLEEP_TIME:
		print "recursive"
...
def durationFromMediaUrl(mediaUrl):
	...
	if os.path.isfile(file_name) :
		...
		curl_mp3 = subprocess.check_output('curl -o "%s" -L "%s"; exit 0' % (file_name, mediaUrl),
											shell = True, stderr = subprocess.STDOUT)
		if not os.path.exists(file_name):
			print "RECEIVED NO CONTENT FROM URL '%s'" % mediUrl			
...
# 		return 0
	...
	try:
		...
		print "The file '" + file_name + "' does not appear to be a valid MP3 file.  REMOVING."
		os.remove(file_name)
		retrieved_file = False
